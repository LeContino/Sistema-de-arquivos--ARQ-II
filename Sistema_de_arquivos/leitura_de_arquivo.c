...
//criação do arquivo
    ...
    else
    {
        ...
        fprintf(fat,"%d \n",pos_cluster_um);  //escreve posição do 1°cluster no arquivo ->inicio_root  e passa para a proxima linha
        ...
        for (i=0;i<padrao;i++){     //qntidade de clusters
            for (j=0;j<tam_cluster;j++) // cada cluster
            {
                ...
                fprintf(fat,"%d",zero); //printa o valor zero de todos os clusters para determinar o tamanho máx do arquivo
            }
        }
    }
